...
class Protocol(FusionApi):
    def __init__(self):
        ...
        img_name_dict, os.path.split(img_name_dict["Path"])[0] = self.get_image_dir()

        self.img_name_dict, self.image_dir = img_name_dict, os.path.split(img_name_dict["Path"])[0]
    ...
    def z_stack(self, img_name="image", wellcoord="0 0", z_increment=5, n_aqcuisitions=10):
        ...
        if self.test is False:
            for i in range(n_aqcuisitions):
                ...
                self.change_image_name(image_name=os.path.join(well_dir, "{}{}_well{}_zheigth{}".
                                                               format(img_name, i + 1, wellcoord, int(z))))
                self.run_protocol() # Delay
        else:
            for i in range(n_aqcuisitions):
                state, _ = self.microscope.get_current_z()
    ...
    def well_image_acquisition(self, img_name="image", wellcoord="0 0", z_increment=5, n_aqcuisitions=10):

        well_dir = self.z_stack(img_name="image", wellcoord="0 0", z_increment=5, n_aqcuisitions=10)

        # Get all recent z stack acquisitions
        img_paths = os.listdir(well_dir)
